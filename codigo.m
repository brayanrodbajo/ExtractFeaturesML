...
addpath('D:\Usuarios\14465809\Desktop\MIRtoolbox1.6.1\MIRToolbox');
addpath('D:\Usuarios\14465809\Desktop\AuditoryToolbox');
clear;
pwd = 'D:\Usuarios\14465809\Desktop\ICESI\Baladas AC';
path_files=getAllFiles(pwd,'*.mp3',1);
namesfiles=(getAllFiles(pwd,'*.mp3',0)); %Names
pathfilesprim=char(path_files); %Paths
numrows=length(path_files)*3; 
numfiles=length(path_files);
pathfiles=cell(numfiles*3,1); %Paths
namesfilescell=cell(numfiles*3,1); %Paths

for i=1:numfiles
    for c=1:3
        namesfilescell((i-1)*3+c) = namesfiles(i);
        pathfiles{(i-1)*3+c} = path_files{i};
    end
end
...
compresionMethodfiles=cell(numrows,1);
titlesfiles=cell(numrows,1);
commentfiles=cell(numrows,1);
artistsfiles=cell(numrows,1);
...
for i=1:numfiles
    for c=1:3
    infofiles(i)=audioinfo(pathfilesprim(i,:));
    file_temp=dir(pathfilesprim(i,:));
    ...
    compresionMethodfiles{(i-1)*3+c} = char(infofiles(i).CompressionMethod);
    titlesfiles{(i-1)*3+c} = char(infofiles(i).Title);
    commentfiles{(i-1)*3+c} = char(infofiles(i).Comment);
    artistsfiles{(i-1)*3+c} = char(infofiles(i).Artist);
    end
end
...
%Use 10s of the song 
ri = round((durationfiles((i-1)*3+1)-10)*rand(1,1));
ri
durationfiles((i-1)*3+c)
file_audio=miraudio(pathfilesprim(i,:),'Extract',ri,ri+10);
mirsave(file_audio, strcat(pathfilesprim(i,:),'part',num2str(c),'.wav'));
pathfilesprim(i,:)
...
T = table(namesfilescell,datesfiles,compresionMethodfiles,bytesfiles,pathfiles,numchannelsfiles,sampleratefiles,totalsamplesfiles,...
    durationfiles,titlesfiles,commentfiles,artistsfiles,bitratefiles,rmsfiles,rmsmedianfiles,lowenergyfiles,ASRfiles,...
    eventdensityfiles,tempofiles,pulseclarityfiles,zerocrossfiles,rolloffsfiles,brightnessfiles,spreadfiles,...
...
%Export to.cvs File
writetable(T,'myDataA.csv','Delimiter',',','QuoteStrings',true);
